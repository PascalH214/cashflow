...
public class JpaAccountRepository implements AccountRepository {
    ...
    public Account save(Account account) {
        ...
        if (accountEntity instanceof BankAccountEntity bankAccountEntity) {
            bankAccountEntity.setCountry(
                    countryJpaRepository.getReferenceById(
                            ((de.pascalh214.cashflow.features.account.domain.BankAccount) account)
                                    .getCountry()
                                    .getId()
                                    .value()
                    )
